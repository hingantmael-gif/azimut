/**
 * Extraction soignée — Platine 1 uniquement (ailes cristal).
 * node scripts/process-platine-1.mjs
 */
...
const SOURCE = join(root, 'assets/ranks/sources/frames/platine-1.png');
const OUT = join(root, 'assets/ranks/badges/platine-1.png');
...
function isNavyCenter(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return b > 55 && b >= g * 0.95 && r < g * 1.05 && r >= 35;
}

function isCyanGem(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return b > 120 && g > 100 && b >= r && g >= r * 0.85;
}

function isLightning(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return g > 90 && b > 110 && g >= r * 0.9;
}

/** Ailes cristal / reflets cyan clairs. */
function isCrystal(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  const spread = Math.max(r, g, b) - Math.min(r, g, b);
  return b > 95 && g > 80 && b >= g * 0.95 && spread < 90;
}
...
function rowStats(data, width, y) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    else if (isGold(r, g, b)) gold++;
    else if (isEmblemPixel(r, g, b)) emblem++;
  }
  return { black, white, emblem, gold };
}
...
function stripBottomContaminants(rgba, width, height) {
  for (let y = height - 1; y >= 0; y--) {
    ...
    const contaminant = gold >= 3 || (textBlack >= 5 && emblem < 6);
    if (contaminant) {
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4;
        rgba[i + 3] = 0;
...
async function main() {
  ...
  console.log(`✓ platine-1.png → ${meta.width}×${meta.height}`);
}
