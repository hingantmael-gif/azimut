/**
 * Extraction soignée — Or 3 uniquement (hexagone).
 * node scripts/process-or-3.mjs
 */
...
const SOURCE = join(root, 'assets/ranks/sources/frames/or-3.png');
const OUT = join(root, 'assets/ranks/badges/or-3.png');
...
/** Centre bronze / cuivre foncé. */
function isDarkGold(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return r >= 70 && r <= 150 && g >= 45 && g <= 115 && b >= 25 && b <= 85 && r > g && g > b;
}

/** Argent (aperçu rang inférieur). */
function isSilver(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b) || isGold(r, g, b) || isDarkGold(r, g, b)) return false;
...
function isEmblemPixel(r, g, b) {
  return isGold(r, g, b) || isDarkGold(r, g, b) || isBlack(r, g, b);
}
...
async function main() {
  ...
  console.log(`✓ or-3.png → ${meta.width}×${meta.height}`);
}
