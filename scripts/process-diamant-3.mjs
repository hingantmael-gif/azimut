/**
 * Extraction soignée — Diamant 3 uniquement (cristal bleu).
 * node scripts/process-diamant-3.mjs
 */
...
const SOURCE = join(root, 'assets/ranks/sources/frames/diamant-3.png');
const OUT = join(root, 'assets/ranks/badges/diamant-3.png');
...
function isDiamondBlue(r, g, b) {
  ...
  return b > 70 && b >= g * 0.92 && g >= r * 0.75;
}

function isLightDiamond(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return b > 140 && g > 120 && b >= g;
}

function isPlatinum(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b) || isDiamondBlue(r, g, b)) return false;
...
function isEmblemPixel(r, g, b) {
  return isDiamondBlue(r, g, b) || isLightDiamond(r, g, b) || isBlack(r, g, b);
}
...
function rowStats(data, width, y) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    else if (isPlatinum(r, g, b)) platinum++;
    else if (isEmblemPixel(r, g, b)) emblem++;
  }
  return { black, white, emblem, platinum };
}
...
function isLogoPixel(r, g, b, a) {
  ...
  if (isPlatinum(r, g, b)) return false;
...
function stripBottomContaminants(rgba, width, height) {
  for (let y = height - 1; y >= 0; y--) {
    ...
    const contaminant = platinum >= 3 || (textBlack >= 5 && emblem < 6);
    if (contaminant) {
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4;
        rgba[i + 3] = 0;
...
/** Retire poussière / aperçu rang supérieur en haut. */
function stripTopContaminants(rgba, width, height) {
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      if (isEmblemPixel(rgba[i], rgba[i + 1], rgba[i + 2])) emblem++;
    }
    if (emblem >= 12) break;
    for (let x = 0; x < width; x++) {
      const i = (y * width + x) * 4;
...
async function main() {
  ...
  console.log(`✓ diamant-3.png → ${meta.width}×${meta.height}`);
}
