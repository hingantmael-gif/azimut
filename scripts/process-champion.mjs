/**
 * Extraction soignée — Champion uniquement (couronne, ailes or, ruban).
 * node scripts/process-champion.mjs
 */
...
const SOURCE = join(root, 'assets/ranks/sources/frames/champion.png');
const OUT = join(root, 'assets/ranks/badges/champion.png');
...
function isRed(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return r > 95 && r > g * 1.2 && r > b * 1.2;
}

function isDarkRed(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return r >= 60 && r <= 140 && g < 70 && b < 70 && r > g && r > b;
}
...
function isEmblemPixel(r, g, b) {
  return isGold(r, g, b) || isRed(r, g, b) || isDarkRed(r, g, b) || isBlack(r, g, b);
}
...
function rowStats(data, width, y) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    else if (isElitePurple(r, g, b)) elite++;
    else if (isEmblemPixel(r, g, b)) emblem++;
  }
  return { black, white, emblem, elite };
}
...
function findCropTop(data, width, cropBottom) {
  for (let y = 0; y < cropBottom; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      else if (isRed(r, g, b) || isDarkRed(r, g, b)) red++;
      else if (isElitePurple(r, g, b)) elite++;
      else if (r < 130 && g < 130 && b < 130) text++;
    }
    if (gold >= 10 && red >= 4 && elite === 0 && text < 20) {
...
function isLogoPixel(r, g, b, a) {
  ...
  if (isElitePurple(r, g, b)) return false;
...
function stripBottomContaminants(rgba, width, height) {
  for (let y = height - 1; y >= 0; y--) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      else if (isBlack(r, g, b) || (r < 130 && g < 130 && b < 130)) textBlack++;
    }

    const contaminant = elite >= 3 || (textBlack >= 5 && emblem < 6);
    if (contaminant) {
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4;
        rgba[i + 3] = 0;
...
async function main() {
  ...
  console.log(`✓ champion.png → ${meta.width}×${meta.height}`);
}
