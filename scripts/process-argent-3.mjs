/**
 * Extraction soignée — Argent 3 uniquement.
 * node scripts/process-argent-3.mjs
 */
...
const SOURCE = join(root, 'assets/ranks/sources/frames/argent-3.png');
const OUT = join(root, 'assets/ranks/badges/argent-3.png');
...
/** Pierre / ardoise bleu-gris du centre. */
function isStone(r, g, b) {
  if (isWhite(r, g, b) || isBlack(r, g, b)) return false;
  return b >= g && g >= r * 0.85 && r >= 45 && b <= 130;
}

/** Bronze (aperçu rang précédent en bas). */
...
function isEmblemPixel(r, g, b) {
  return isSilver(r, g, b) || isStone(r, g, b) || isBlack(r, g, b);
}
...
function isTextRow(data, width, y) {
  ...
  return white / width >= 0.85 && black >= 8 && emblem < 18;
}
...
async function main() {
  ...
  console.log(`✓ argent-3.png → ${meta.width}×${meta.height}`);
}
